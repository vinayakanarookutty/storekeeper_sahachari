...
export default function BookingsScreen() {
  ...
  const renderActionButtons = (booking: any) => {
    ...
    const isThisBookingPending = anyPending && updateStatusMutation.variables?.bookingId === booking._id;
    const pendingStatus = isThisBookingPending ? updateStatusMutation.variables?.nextStatus : null;
        ...
        {booking.status === 'ACCEPTED' && (
          ...
          >
            <LinearGradient colors={['#2196F3', '#1976D2']} style={screenStyles.actionButtonGradient}>
              {pendingStatus === (isService ? 'IN_PROGRESS' : 'COMPLETED') ? (
                <ActivityIndicator size="small" color="#fff" />
              ) : (
                <Text style={screenStyles.actionButtonText}>
